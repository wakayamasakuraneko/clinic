...
tables.forEach((table) => {
  ...
  // ↑　table内のtableにはコードに記述がなくても自動でtbodyが作られる。
  // そのためtbodyだけはクラスを授けてそれを取得しないと意図しないtbodyまで反応してしまう。
  ...
  heads.forEach((head, index) => {
    head.addEventListener("click", () => {
      ...
      window.dispatchEvent(new Event("resize")); // 行間再判定
    });
  ...
  for (let i = 0; i < taxStatus.length; i++) {
    ...
    switch (taxStatusText) {
      ...
      case "抜/込":
        ...
        taxStatus[
          i
        ].innerHTML = `<span style="color: blue;">${parts[0]}</span>/<span style="color: red;">${parts[1]}</span>`;
        break;
      default:
        // デフォルトの設定（必要に応じて変更）
